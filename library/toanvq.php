...
function getAudioFile($text, $name = null)
{
	$output = null;
	...
	try {
		...
		if($mime_type == "audio/x-wav"){
			$date = date("Y-m-d");
			$dir_of_date = $dir_path.$date;
			if(!is_dir($dir_of_date)){
				mkdir($dir_of_date);
			}
			$final_path = $dir_of_date."/wav/";
			...
			if(file_put_contents($file_path, $body)){
				$output = str_replace(".", "", $final_path).$file_name;
			}
		}else{
			return $body;
		}
...
function getFile($url, $save_path = "/assets/download/")
{
...
function navibar($main_selected = -1, $sub_selected = -1)
{
	$selected = array($main_selected, $sub_selected);
		$big_type = new Model_Bigtype;
		$big_type_items = array();
		foreach ($big_type->getAll() as $key => $type) {
			$big_type_items[] = array("url" => "/admin/manage/source?type=$type[id]", "title" => $type['title'], 'mark' => $type['id']);
		}
	$items = array(
						array(  'title' => 'Dashboard',
                      	...
                      	array(  'title' => 'Resources',
	                            ...
	                            'items' => array(
	                                                ...
	                                                array("url" => "/admin/resource/types", "title" => 'Resource Types', 'mark' => 2),
	                                                array("url" => "/admin/resource/links", "title" => 'Resource Links', 'mark' => 3),
	                                                array("url" => "/qplayvn/index", "title" => 'QPlayVN - Old System', 'mark' => 4),
	                                            )
	                          ),
                        array(  'title' => 'Manage Content',
	                            'url'  => '#',
	                            'icon'  => 'fa-bar-chart-o',
	                            'mark'  => 3,
	                            'items' => $big_type_items
	                          ),
