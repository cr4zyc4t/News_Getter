...
class Vqt_Controller_Api extends Zend_Controller_Action
{
	private $ctg_id = null;
	public $reply = array();
	public $test_mode = FALSE;
	
	function __construct(Zend_Controller_Request_Abstract $request, Zend_Controller_Response_Abstract $response, array $invokeArgs = array(), $ctg_id = null){
	...
	{
		if (is_numeric($this->ctg_id)) {
			$subcategory_model = new Model_Subcategory;
			$list = $subcategory_model->getList($this->ctg_id);
	...
	public function searchAction()
	{
		...
		if ($keyword) {
			...
			$list = $content_model->seachContent($keyword, $model_subcategory->getListSubCtg($this->ctg_id),$count, $count*( $page-1 ));
	...
	public function getHighlightAction()
	{
		...
		$this->reply['success'] = TRUE;
		$this->reply['video_list'] = $model_content->getHighLightByCategory($this->ctg_id, 9, 9*( $page-1 ));
	}
