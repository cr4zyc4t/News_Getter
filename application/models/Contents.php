<?php
class Model_Contents extends Zend_Db_Table_Abstract
{
	public $_name = 'news';
	...
	public function getNoAudioNews()
	{
		return $this->getDefaultAdapter()->fetchRow("SELECT * FROM $this->_name WHERE DATE(created_time) = DATE(NOW()) AND wav='' AND type=1");
	}
	
	public function getNoMp3News()
	{
		return $this->getDefaultAdapter()->fetchRow("SELECT * FROM $this->_name WHERE DATE(created_time) = DATE(NOW()) AND wav <> '' AND wav <> 'false' AND mp3 = '' AND type=1");
	}
