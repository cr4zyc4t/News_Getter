...
class Api_5SonlineController extends Zend_Controller_Action
{
	private $reply = array();
	...
	public function getcontentsAction()
	{
		...
		if (is_numeric($subcategory_id)) {
			...
			foreach ($list as $key => $value) {
				// echo html_entity_decode($list[$key]['content']); die;
				$list[$key]['content'] = html_entity_decode($value['content'], ENT_QUOTES, "utf-8");
				$list[$key]['content_1'] = html_entity_decode($value['content_1'], ENT_QUOTES, "utf-8");
				$list[$key]['content_2'] = html_entity_decode($value['content_2'], ENT_QUOTES, "utf-8");
				$list[$key]['content_3'] = html_entity_decode($value['content_3'], ENT_QUOTES, "utf-8");
			}
	...
	public function searchAction()
	{
		...
		if ($keyword) {
			...
			$content_model = new Model_Contents;
			$list = $content_model->seachContent($keyword, array(84,87,86,176),$count, $count*( $page-1 ));
	...
	public function getHighlightAction()
	{
		...
		$this->reply['success'] = TRUE;
		$this->reply['video_list'] = $model_content->getHighLightByCategory(31, 9, 9*( $page-1 ));
	}
